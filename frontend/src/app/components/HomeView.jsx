...
export function HomeView({ songs = [], currentSong, isPlaying, onSongSelect, user, onLogout, onNavChange, themeColor = "#00ffcc" }) {
  ...
  return (
     ...
     >
       <main className="px-10 py-6 flex flex-col gap-12">
             ...
             <div className="flex flex-col gap-24">
                <section>
                   ...
                   <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      >
                         <img src={forYou.big?.imageUrl} className="w-full h-full object-cover transition-transform duration-[2s] group-hover:scale-110" alt="" />
                         ...
                         {forYou.small.map(song => (
                           ...
                           >
                              <div className="w-20 h-20 rounded-[20px] overflow-hidden shadow-2xl border border-foreground/[0.05] relative">
                                 <img src={song.imageUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="" />
                ...
                <section className="relative px-20 py-24 rounded-[64px] overflow-hidden border border-primary/20 group text-center bg-card shadow-2xl">
                   ...
                   <div className="relative z-10">
                      ...
                      <div className="flex items-center justify-center gap-8">
                         <button className="h-20 px-16 rounded-[28px] bg-primary text-primary-foreground text-[14px] font-black uppercase tracking-[4px] hover:scale-105 transition-all shadow-[0_0_50px_rgba(var(--primary-rgb),0.3)]">Start Session</button>
                         <button className="h-20 px-16 rounded-[28px] border-2 border-foreground/[0.1] text-[14px] font-black uppercase tracking-[4px] hover:bg-foreground/[0.03] transition-all">View All Vibes</button>
                      </div>
                   </div>
                </section>
                      ...
                      {continueListening.map((song, i) => (
                        <div key={song.id} onClick={() => onSongSelect(song)} className="w-[200px] flex-none group cursor-pointer">
                           <div className="relative aspect-square rounded-[40px] overflow-hidden mb-6 border border-foreground/[0.05] shadow-2xl">
                              <img src={song.imageUrl} className="w-full h-full object-cover grayscale-[0.3] group-hover:grayscale-0 group-hover:scale-110" alt="" />
                      ...
                      {trending.map((song, i) => (
                        <div key={song.id} onClick={() => onSongSelect(song)} className="w-[360px] flex-none group cursor-pointer relative">
                           <div className="aspect-[4/5] rounded-[56px] overflow-hidden shadow-2xl border border-foreground/[0.05] mb-8">
                              <img src={song.imageUrl} className="w-full h-full object-cover transition-transform duration-[3s] group-hover:scale-110" alt="" />
                ...
                <section>
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-10">
                      {aiPicks.map((song, i) => (
                        <motion.div key={song.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
                ...
                <section className="pb-40">
                   <div className="flex flex-col items-center gap-12 text-center">
                      <div className="flex items-center gap-8 w-full">
                         <div className="flex-1 h-[1px] bg-foreground/[0.05]" /><span className="text-[12px] font-black uppercase tracking-[10px] opacity-20 font-sans">Browse Architecture</span><div className="flex-1 h-[1px] bg-foreground/[0.05]" />
                      </div>
                      <div className="flex items-center justify-center flex-wrap gap-6 max-w-4xl">
                         {["Abstract Soul", "Nu-Disco", "Cyber Pop", "Hardcore", "Noir Jazz", "Techno Deep"].map(genre => (<button key={genre} className="px-14 h-20 rounded-[32px] bg-foreground/[0.02] border border-foreground/[0.05] text-[15px] font-black uppercase tracking-[5px] hover:bg-foreground hover:text-background transition-all shadow-xl">{genre}</button>))}
                      </div>
                   </div>
                </section>
             </div>
